...
task :hashnode do
  ...
  token = ENV.fetch('HASHNODE_TOKEN', nil)
  publication_id = ENV.fetch('HASHNODE_PUBLICATION_ID', nil)
  ...
  markdown_body = markdown_body.gsub(%r{(!\[[^\]]*\]\()(/[^)]+)\)}) do
    "#{Regexp.last_match(1)}#{site_url}#{Regexp.last_match(2)})"
  end
